...
sam <- function(model            = NULL,
    ...
    STEP2 <- lav_sam_step2(STEP1 = STEP1, FIT = FIT,
    ...
    JOINT <- lav_sam_step3_joint(FIT = FIT, PT = STEP2$PT,
                                 sam.method = sam.method)
    ...
    VCOV <- lav_sam_step2_se(FIT = FIT, JOINT = JOINT, STEP1 = STEP1,
    ...
    if(output == "lavaan") {
        ...
        SAM <- lav_sam_table(JOINT = JOINT, STEP1 = STEP1,
                             FIT.PA = STEP2$FIT.PA,
                             mm.args = mm.args,
