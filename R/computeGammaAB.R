...
compute.Abeta.complete <- function(Sigma.hat=NULL, Mu.hat=NULL, 
    ...
    D <- duplication.matrix( ncol(Sigma.hat) ); tD <- t(D)
...
compute.A1.sample <- function(sample, group=1, meanstructure=TRUE,
    ...
    if(sample@missing.flag) {
        ...
        D <- duplication.matrix( ncol(sample.icov) ); tD <- t(D)
...
compute.B1.sample <- function(sample=NULL, group=1, meanstructure=TRUE) {
    ...
    if(sample@missing.flag) {
        ...
        Gamma <- compute.Gamma(data=sample@data.obs[[group]],
                               meanstructure=meanstructure)
        B1 <- A1 %*% Gamma %*% A1
    }
