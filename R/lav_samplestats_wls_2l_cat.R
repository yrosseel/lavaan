...
lav_samp_wls_2l_cat <- function(lavsamplestats = NULL,
  ...
  for (g in seq_len(ngroups)) {
    ...
    ov_idx_1 <- lp$ov.idx[[1]]
    ov_idx_2 <- lp$ov.idx[[2]]
    if (!identical(sort(ov_idx_1), sort(ov_idx_2))) {
      lav_msg_stop(gettext(
        "two-level (D)WLS estimation with categorical data requires (for
        now) that all variables appear at both levels (no within-only or
    ...
    # SIGMA.W is 1 for the ordinal variables)
    h1_implied$cov[[(g - 1) * nlevels + 1L]] <- m07$SIGMA.W
    h1_implied$cov[[(g - 1) * nlevels + 2L]] <- m07$SIGMA.B
    h1_implied$mean[[(g - 1) * nlevels + 1L]] <-
      numeric(NROW(m07$SIGMA.W))
    h1_implied$mean[[(g - 1) * nlevels + 2L]] <- m07$MU
  }
...
# and its ACOV into the lavaan two-level WLS layout described above
lav_m07_repackage <- function(m07 = NULL, ov_types = NULL) {
  ...
  # --- within block ---
  # 1. th: thresholds (ordinal); -mu (continuous, fixed 0 at within level)
  ...
  # 3. within covariances/'correlations' (vech, no diagonal): cw
  # pair pp in column-major lower-tri order = position ntheta + 2(pp-1)+1
  for (pp in seq_len(pstar)) {
    src <- c(src, ntheta + 2L * (pp - 1L) + 1L)
    fixed_val <- c(fixed_val, NA_real_)
  }
  ...
  # 5. vech(Sigma_b, diagonal = TRUE): vb (diag) and cb (off-diagonal)
  # in lav_mat_vech (column-major) order
  ...
  for (j in seq_len(nvar)) {
    for (i in j:nvar) {
      if (i == j) {
        ...
        src <- c(src, ntheta + 2L * (pair_pos[i, j] - 1L) + 2L) # cb
        fixed_val <- c(fixed_val, NA_real_)
      }
    }
  }
  ...
  nth_all <- sum(th_idx > 0L) + sum(th_idx == 0L)
