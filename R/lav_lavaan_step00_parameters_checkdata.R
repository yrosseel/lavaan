lav_lavaan_step00_parameters <- function(matchcall, defaults, syscall, dotdotdot) {
  ...
  #   gradient -> optim.gradient !!! overwritten by dotdotdot$gradient if present
  #   init_nelder_mead -> optim.init_nelder_mead
  mc <- matchcall
  sc <- syscall
  ddd <- dotdotdot
  # catch partial matching of 'cl' (expanded to cluster)
  if (!is.null(sc[["cl"]]) && is.null(sc[["cluster"]]) && !is.null(mc[["cluster"]])) {
    mc[["cl"]] <- mc[["cluster"]]
    mc[["cluster"]] <- NULL
    ddd$cl <- sc[["cl"]]
  }
  ...
  if (!is.null(ddd$control)) {
    ...
    if (!is.null(ddd$control$optim.force.converged)) {
      ddd$optim.force.converged <-
        ddd$control$optim.force.converged
    }
    # gradient
    ...
    if (!is.null(ddd$control$init_nelder_mead)) {
      ddd$optim.init_nelder_mead <-
        ddd$control$init_nelder_mead
    }
  }
  return(list(mc = mc, dotdotdot = ddd))
}
lav_lavaan_step00_checkdata <- function(data, dotdotdot, sample.cov, sample.nobs,
                                      sample.mean, sample.th, NACOV, WLS.V, ov.order) {
  # if data not NULL:
  #   if it is an 'enriched' data.frame (e.g. a tibble), simplify to ordinary data.frame
  #   if it has class 'lavMoments':
  ...
  #             if lavOptions present in data, copy those that are not provided in the function call
  #                   to dotdotdot$...
  #             set data to NULL
