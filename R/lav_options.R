  ...
  if (!is.null(x)) {
    if (is.character(x)) {
      ...
      if (length(not.ok) > 0L) {
        ...
        if (length(x) > 1L) {
          warning(
            "lavaan WARNING: option `", x[not.ok],
            "' not available"
          )
        }
        x <- x[-not.ok]
...
lav_options_set <- function(opt = NULL) {
  ...
  if (length(opt$group.equal) > 0L && opt$group.equal[1] == "none") {
    ...
    # nothing to do
  } else if (all(opt$group.equal %in% c(
    ...
    stop(
      "lavaan ERROR: unknown value for `group.equal' argument: ",
  ...
  if (opt$.multilevel) {
    ...
    if (opt$se == "default") {
      ...
      "none", "standard", "robust.huber.white",
      "sandwich"
    )) {
      # nothing to do
    } else if (opt$se == "robust") {
  ...
  if (opt$missing == "default") {
    ...
    if (opt$.categorical) {
      stop(
        "lavaan ERROR: missing = ", dQuote(opt$missing),
        " not available in the categorical setting"
      )
    }
    opt$missing <- "ml"
    if (opt$estimator %in% c(
      "mlm", "mlmv", "gls", "wls", "wlsm", "wlsmv",
      "uls", "ulsm", "ulsmv", "pml", "dls"
    )) {
      stop(
        "lavaan ERROR: missing=\"ml\" is not allowed for estimator ",
  ...
  } else if (opt$missing %in% c(
    "two.stage", "twostage", "two-stage",
    ...
    if (opt$estimator %in% c(
      ...
      stop(
        "lavaan ERROR: missing=\"two.stage\" is not allowed for estimator ",
        dQuote(toupper(opt$estimator))
      )
    }
  } else if (opt$missing %in% c(
    "robust.two.stage", "robust.twostage",
    ...
    if (opt$estimator %in% c(
      ...
      stop(
        "lavaan ERROR: missing=\"robust.two.stage\" is not allowed for estimator ",
        dQuote(toupper(opt$estimator))
      )
    }
  ...
  if (opt$missing %in% c("ml", "ml.x") && opt$se == "robust.sem") {
    warning(
      "lavaan WARNING: missing will be set to ",
  ...
  if (opt$missing == "two.stage" ||
    ...
    if (opt$se == "default") {
      ...
      warning(
        "lavaan WARNING: se will be set to ",
    ...
    if (opt$information[1] == "default") {
      ...
      warning(
        "lavaan WARNING: information will be set to ",
    ...
    if (length(opt$test) == 1L && opt$test == "default") {
      ...
      warning(
        "lavaan WARNING: test will be set to ",
  ...
  if (opt$estimator %in% c("ml", "mlf", "mlm", "mlmv", "mlmvs", "mlr")) {
    ...
    if (length(bad.idx) > 0L) {
      stop(
        "lavaan ERROR: invalid value(s) in test= argument when estimator is GLS:\n\t\t",
    ...
    if (length(bad.idx) > 0L) {
      stop(
        "lavaan ERROR: invalid value(s) in test= argument when estimator is NTRLS:\n\t\t",
        paste(opt$test[bad.idx], collapse = " "), "\n"
      )
    }
    ...
    if (length(bad.idx) > 0L) {
      stop(
        "lavaan ERROR: invalid value(s) in test= argument when estimator is WLS:\n\t\t",
        paste(opt$test[bad.idx], collapse = " "), "\n"
      )
    }
    ...
    if (length(bad.idx) > 0L) {
      stop(
        "lavaan ERROR: invalid value(s) in test= argument when estimator is DLS:\n\t\t",
        paste(opt$test[bad.idx], collapse = " "), "\n"
      )
    }
    ...
    stop(
      "lavaan ERROR: unknown value for estimator= argument: ",
      opt$estimator, "\n"
    )
  }
  ...
  if (opt$link == "logit") {
    if (opt$estimator != "mml") {
      warning(
        "lavaan WARNING: link will be set to ",
  ...
  if (!opt$estimator %in% c("ML", "REML", "PML", "FML", "NTRLS", "catML")) {
    ...
    if (opt$estimator %in% c("PML", "FML")) {
      ...
      stop(
        "lavaan ERROR: invalid value for `likelihood' argument: ",
        opt$likelihood, "\n"
      )
    }
  ...
  if (is.logical(opt$effect.coding)) {
    ...
    # nothing to do
  } else if (all(opt$effect.coding %in% c(
    "loadings", "intercepts",
    "mg.lv.efa.variances",
    "mg.lv.variances",
    "mg.lv.means",
    ...
    stop(
      "lavaan ERROR: unknown value for ", sQuote("effect.coding"),
  ...
  if (length(wrong.idx) > 0L) {
    txt <- c(
      "invalid option(s) for test argument: ",
  ...
  if (length(opt$optim.bounds) > 0L) {
    ...
    if (opt$bounds %in% c("default", "none", "user")) {
      opt$bounds <- "user"
    } else {
      stop(
        "lavaan ERROR: bounds and optim.bounds arguments can not ",
        "be used together;\n\t\t",
  ...
  if (opt$bounds == "none") {
    ...
    if (length(opt$optim.bounds) == 0L) {
      stop(
        "lavaan ERROR: bounds= is ", dQuote("user"),
        " but optim.bounds= argument is empty"
      )
    }
  ...
  if (opt$rotation %in% c(
    "crawfer", "crawford.ferguson", "crawford-ferguson",
    "crawfordferguson"
  )) {
    opt$rotation <- "cf"
  }
  if (opt$rotation %in% c(
    "varimax", "quartimax", "orthomax", "cf", "oblimin",
    "quartimin", "geomin", "entropy", "mccammon", "infomax",
    "tandem1", "tandem2", "none", "promax",
    "oblimax", "bentler", "simplimax", "target", "pst"
  )) {
    # nothing to do
  } else if (opt$rotation %in% c(
    ...
    txt <- c(
      "Rotation method ", dQuote(opt$rotation), " not supported. ",
  ...
  if (opt$rotation.args$row.weights == "default") {
  ...
  } else if (opt$rotation.args$row.weights %in% c(
    "cureton-mulaik",
    "cureton.mulaik", "cm"
  )) {
    ...
    stop(
      "lavaan ERROR: rotation.args$row.weights should be \"none\",",
  ...
  if (opt$rotation.args$order.lv.by %in% c("sumofsquares", "index", "none")) {
    ...
    stop(
      "lavaan ERROR: rotation.args$order.lv.by should be \"none\",",
...
lav_options_check_se <- function(opt = NULL) {
  ...
  if (!opt$se %in% c(
    ...
    stop(
      "lavaan ERROR: invalid value in se= argument:\n\t\t",
      dQuote(opt$se)
    )
  }
