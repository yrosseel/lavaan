...
lav_mvnorm_cluster_missing_loglik_samplestats_2l <- function(Y1   = NULL,
    ...
    if(nz > 0L) {
        ...
        for(p in seq_len(Zp$npatterns)) {
            freq <- Zp$freq[p]; z.na.idx <- which(!Zp$pat[p,])
    ...
    PJ <- rowsum.default(PIJ[,both.idx], cluster.idx, reorder = FALSE,
                         na.rm = TRUE) # only both part is needed
...
lav_mvnorm_cluster_missing_dlogl_2l_samplestats <-
    ...
    if(nz > 0L) {
        ...
        ZIZY      <- rep(list(matrix(0, nrow(sigma.zy),
                                        ncol(sigma.zy))), Zp$npatterns + 1L)
        ZIP       <- rep(list(matrix(0, nrow(sigma.zz),
                                        ncol(sigma.zz))), Zp$npatterns + 1L)
        for(p in seq_len(Zp$npatterns)) {
            freq <- Zp$freq[p]; z.na.idx <- which(!Zp$pat[p,])
            j.idx <- Zp$case.idx[[p]] # cluster indices with this pattern
            ZPAT2J[j.idx] <- p
    ...
    PJ <- rowsum.default(PIJ[,,drop = FALSE],
                         cluster.idx, reorder = FALSE, na.rm = TRUE)
    ...
    for(j in seq_len(nclusters)) {
        ...
        if(nz > 0L) {
            g.j <- as.matrix(GJ[j,]); zij <- as.matrix(GZ[j,])
            zizy      <- ZIZY[[ ZPAT2J[j] ]]
            zip       <- ZIP[[  ZPAT2J[j] ]]
        ...
        if(length(which.incompl) > 0L) {
           p.idx <- MPi[ij.index][which.incompl]
            tmp <- lapply(WIP[ p.idx ], function(x) { x %*% PART1 %*% x})
            AP1A.b <- Reduce("+", tmp)
            AP2[which.incompl] <-
                lapply(WIP[ p.idx ], function(x) { x %*% PART2 })
