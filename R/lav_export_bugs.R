...
lav2bugs <- function(partable, pta = NULL, as.function.=FALSE) {
    ...
    for(g in 1:nblocks) {
        ...
        ov.names.nox <- vnames$ov.nox[[g]]; nov <- length(ov.names.nox)
        TXT <- paste(TXT, "\n", t2,
            "# ov.nox", sep="")
        for(i in 1:nov) {
        ...
        for(i in 1:nov) {
            ...
            if(length(int.idx) == 1L) {
                ...
                if(partable$free[int.idx] == 0L) {
                    ...
                    TXT <- paste(TXT,
                        "theta[", partable$free[int.idx], "]", sep="")
                }
            ...
            for(j in r.idx) {
                ...
                if(rhs %in% lv.names) {
                    ...
                    RHS <- paste("y", g, "[i,",
                                 match(rhs, ov.names), "]", sep="")
                }

                # fixed or free?
                if(partable$free[j] == 0L) {
                    TXT <- paste(TXT, " + ",
                        partable$ustart[j], "*", RHS, sep="")
                } else {
                    TXT <- paste(TXT, " + ",
                        "theta[", partable$free[j], "]*", RHS, sep="")
                }
            }
        }
    ...
    TXT <- paste(TXT, "\n", t1,
                "# Priors free parameters (univariate):", sep="")
    ...
    for(i in seq_len(npt)) {
        ...
        if(op == "=~") {
            # factor loading
            TXT <- paste(TXT, "\n", t1,
                "theta[", free.idx, "] ~ dnorm(0.8, 1)", sep="")
        } else if(op == "~") {
            # regression
            TXT <- paste(TXT, "\n", t1,
        ...
        if(length(nx) > 0L) {
            ...
            for(idx in lv.phi.idx) {
                TXT <- paste(TXT, "\n", t1,
                    ...
                    match(partable$rhs[idx], vnames$lv.x[[g]]), "]", sep="")
            }
        }
    }
